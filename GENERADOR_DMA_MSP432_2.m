...
ADC_TASKS = 512;    % The number of ADC samples you need 
a = 1;
...
L{a} = '#define ADC_TASKS               (NUMBER_OF_SAMPLES/4)+2';a = a+1;
L{a} = ' ';a = a+1;
...
fid1 = fopen('D:\PROYECTOS2018\PROY_CODE_COM\MSP432_DMA_MULTI7\preCompile.h','wt');
for e = 1:a-1
    fprintf(fid1,'%s\n',L{e});
end

for e = 0:(ADC_TASKS/4)-1
    
    DEST = strcat('        UDMA_DST_INC_32, &dataSet_01[',num2str(e*4));
    DEST = strcat(DEST,'][0],');
...
for e = 0:(ADC_TASKS/4)-1
    ...
    DEST = strcat('        UDMA_DST_INC_32, &dataSet_02[',num2str(e*4));
    DEST = strcat(DEST,'][0],');
    fprintf(fid1,'%s\n','DMA_TaskStructEntry(32, UDMA_SIZE_32,');
    fprintf(fid1,'%s\n','        UDMA_SRC_INC_32,(void*)&ADC14->MEM[0],');
    fprintf(fid1,'%s\n',DEST);
